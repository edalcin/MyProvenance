...
- **Desenha o diagrama de linhagem automaticamente**, ao vivo, à medida que você registra as etapas — sem precisar desenhar nada à mão. Alterne a orientação vertical (padrão)/horizontal com um clique.
...
- **Compartilha um Registro para edição com outra Conta** — convide outra Conta registrada pelo nome de usuário e escolha o papel: **Editor** (edita título, descrição e Atividades/Entidades) ou **Administrador** (paridade total, inclusive Finalizar, Excluir e gerenciar quem mais tem acesso). O Registro compartilhado passa a aparecer também na lista de quem recebeu o convite, com indicação de quem é o dono. Cada coeditor pode sair do compartilhamento quando quiser.
...
- Compartilhamento de edição entre Contas segue hierarquia de papéis (Dono > Administrador > Editor) — quem não é dono nem administrador não finaliza, exclui, nem gerencia quem mais tem acesso; toda checagem de acesso a um Registro retorna 404 (não 403) quando não há permissão, para não vazar existência a quem não tem acesso.
- **Área administrativa** (`/admin`) fica oculta e inacessível sem `ADM_PWD` configurada; mesmo rate limit de 5 tentativas/15 min do login de Conta. Upload de **Restaurar backup** é validado antes de tocar o banco vivo (`PRAGMA integrity_check` + confere a tabela `registros`) e roda em transação — qualquer inconsistência desfaz tudo, sem corromper o banco (ADR-0012).
