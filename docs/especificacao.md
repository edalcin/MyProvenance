...
Painel global (`/admin`), separado do modelo de Conta — protegido por uma única senha na variável de ambiente `ADM_PWD` (sem essa variável configurada, o link "Admin" fica oculto e a área inteira é inacessível). Sessão de admin em memória do processo (token opaco, mesmo gerador de `sessoes.token_hash`), cookie `httpOnly`/`secure`/`sameSite=lax`, sem expiração deslizante — persiste até logout ou reinício do processo. Rate limit de 5 tentativas por 15 min, mesmo mecanismo de `/auth/entrar`.

Gestão global (todas as Contas, todos os Registros/Agentes/compartilhamentos, sem escopo por `usuario_id`) mais backup/restauração do SQLite:
...
| `/admin/restaurar`         | POST         | substitui todos os dados atuais pelo conteúdo de um arquivo `.sqlite` enviado                  |

**Restauração sem reiniciar o processo**: os repositories preparam `Statement`s do `better-sqlite3` uma única vez, no import do módulo, presos à conexão SQLite original — trocar o arquivo em disco por baixo não os atualizaria (eles continuariam apontando para a conexão antiga). Por isso a restauração acontece dentro da mesma conexão viva: o arquivo enviado é validado isoladamente (conexão própria, `readonly`, `PRAGMA integrity_check` + confirma a existência da tabela `registros`), depois anexado (`ATTACH DATABASE`) como schema `origem` na conexão principal, e o conteúdo de cada tabela é trocado (`DELETE` + `INSERT ... SELECT`) dentro de uma única transação, com `foreign_keys` desligado durante a troca e `PRAGMA foreign_key_check` rodando _dentro_ da mesma transação — qualquer inconsistência desfaz tudo (rollback automático), sem corromper o banco vivo. Schema/conexão nunca mudam, só os dados — os `Statement`s já preparados continuam válidos. Ver ADR-0012.

Upload de `.sqlite` exige `BODY_SIZE_LIMIT` do adapter-node maior que o padrão de 512K (`Dockerfile` já define `20M`).

## 10. Referências

- `CONTEXT.md` — glossário (linguagem ubíqua).
- `docs/adr/0001` a `0012` — decisões arquiteturais e seus porquês.
